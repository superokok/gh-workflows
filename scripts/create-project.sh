...
# 2026-09-17 새 저장소 하나를 이 상태로 만들었다 — 프로파일 주석에 "비우면 알림이
...
# (2026-09-16 한 소비 저장소 dry-run: claude-agent·claude-fix·claude.yml·enable-auto-merge가
...
if [ "$DRY" = 1 ] && ! gh repo view "$REPO" >/dev/null 2>&1; then
  ...
  if ! git -C "$WORK/repo" checkout -q "${V[INTEGRATION_BRANCH]}" 2>/dev/null; then
    ...
    # 남는다(2026-09-17 새 저장소를 실제로 만들면서 밟았다).
    #
...
# 루프도 PR #1에서 그대로 멈춘다. 한 소비 저장소가 2026-09-11에 정확히 이걸로 막혔고, 그 교훈이
...
if [ -n "$CHECKS_LATER" ]; then
  ...
  say "     첫 PR부터 머지가 막힌다(2026-09-11 실측). ci.yml을 쓰고 **PR에서 그"
