...
원본은 **실제로 돌던 한 프로젝트**에서 시행착오로 만들어진 것이고, 이 저장소는 **트리거(`on:`) 블록만
...
**통합 브랜치와 운영 브랜치를 둘 다 보호한다.** 운영 쪽에는 필수 체크를 걸지 않는다 —
릴리스 PR은 사람이 보고 머지하는 자리라, 체크가 안 생기면 사람도 못 머지하게 되는 위험만
늘기 때문이다. 거는 건 PR 필수 · force push 차단 · 삭제 차단 셋이다.
(2026-09-17까지는 운영 보호가 `--protect-production` 옵트인이었고 `create-project.sh`가 그걸
안 넘겨서 **새 프로젝트의 운영 브랜치가 매번 무보호로 나왔다.** 지금은 기본이고, 끄려면
`--no-protect-production`.)
...
삭제된다**(2026-09-05 한 소비 저장소에서 실제로 겪고 복구함). 브랜치 정리는 `after-merge.yml`이
base와 이름을 보고 안전하게 한다.

> **템플릿 저장소로 굳히는 건 아직이다.** 표본이 한 프로젝트뿐이라, 무엇이 진짜 공통이고
...
| `claude-review.yml` | `claude-review.yml@main` | `pull_request: {types: [opened, synchronize, ready_for_review, reopened]}` — `notify-handle`을 쓰면 `workflow_run: {workflows: ["<그 저장소 CI 워크플로우의 name>"], types: [completed]}`도 같은 `on:`에 추가한다(`notify-ready` job의 폴백 트리거, 아래 "알아둘 것" 참고). ⚠️ **목록에 이 워크플로우 자신의 `name`은 넣지 않는다** — 자기 참조는 워크플로우 파일을 무효로 만들어 run이 아예 안 생기고, 필수 체크 `review / review`가 영구 Pending으로 굳는다(#63 실측. `actionlint`는 통과시킨다). `review` job 자신의 죽음은 **`claude-fix` 쪽** `workflows:` 목록에 이 이름을 넣어서 본다 — 이 저장소가 `self-fix.yml`로 하는 것이 그것이다. ⚠️ **이름은 그 저장소의 실제 워크플로우 `name`과 정확히 같아야 한다** — 안 맞으면 트리거가 한 번도 안 뜨는데 **아무 신호도 나지 않는다**(project-template에 `"CI"`로 박혀 있었으나 실제 이름은 `Test`였다, 2026-09-17 발견) |
| `claude-agent.yml` | `claude-agent.yml@main` | `issues: {types: [labeled]}` |
| `claude-fix.yml` | `claude-fix.yml@main` | `workflow_run: {workflows: ["<그 저장소 CI의 name>", "<리뷰 호출부의 name>"], types: [completed]}` + `status:`. ⚠️ **리뷰 호출부의 `name`을 이 목록에 넣는다** — `claude-review`는 자기 자신을 감시할 수 없으므로(위 행의 ⚠️) `review` job의 startup_failure는 **여기서만** 잡힌다. 이 저장소가 `self-fix.yml`로 하는 것이 그것이다. `base-branch` 입력(기본값 `"develop"`)을 통합 브랜치에 맞게 넘긴다 — 안 넘기면 `base=main`인 저장소에서는 대상 PR을 영원히 못 찾는다(#60) |
...
- **동기화 PR**(`chore/template-sync`, 입력 `skip-head-ref-prefix`) — 그 diff는 정본 저장소에서
  **이미 리뷰를 거친 파일의 바이트 복사**다. 같은 diff를 소비 저장소 수만큼 다시 리뷰하면
  ...
  74%(이 저장소) · 47% · 31%(소비 저장소 둘)를 차지한다.
...
실측(2026-09-15, 양쪽 `develop`): 소비 저장소 두 곳의 훅 3종 210줄은 바이트 단위로
...
> `plan-only-paths: prisma/`)은 원본 프로젝트(Node + Prisma + Next.js) 기준이다. `with:`를 통째로 생략하면
> **그 기본값이 조용히 상속되고, 실패는 에이전트가 아니라 그 앞 준비 단계에서 난다** —
> 액션이 시작조차 못 하므로 이슈는 라벨만 붙은 채 방치되고 아무 알림도 없다.
> 스택이 다른 한 저장소(JVM 백엔드 + Next.js + 모바일)가 2026-09-12에 정확히 이걸로 멈췄다:
...
- **재사용 워크플로우를 *삭제*할 때는 순서가 반대다.** 추가·수정은 여기를 먼저 고치고 소비
  ...
  (2026-09-09 실제 발생: `auto-merge.yml`을 여기서 먼저 지워 그 소비 저장소의 `main`이
...
- **호출부 job은 `permissions`를 반드시 명시한다.** 불린 워크플로우가 요구하는 권한을
  ...
  `claude-fix` 셋이다. 2026-09-07 한 소비 저장소에서 이걸 빠뜨려 Claude Review가 전부 죽었고,
...
- **재개는 `@claude` 코멘트가 기본 경로다.** 사용자가 라벨 규칙을 외우게 하지 않는다 —
  ...
  워크플로우(`claude.yml`)를 갖고 있을 때의 얘기이고, 없는 저장소에서는
...
- **감시용 cron 워크플로우는 두지 않는다(2026-09-07 `stale-sweep.yml` 삭제).** 위 두 사고는 전부
  ...
  **대체 수단은 소비 프로젝트의 세션 시작 브리핑이다** — 모든 작업이 Claude 세션을 거치므로, 세션이
  ...
  Actions가 통째로 멈춰도 동작한다(소비 저장소의 `CLAUDE.md` "📢 알림" 참고).
...
- **담당자 지정(`notify-handle`)은 체크가 전부 초록이 되거나, 필수 체크가 실패로 끝나
  ...
  **빨간불 알림도 "체크가 전부 끝난 뒤"에 간다(#66).** 예전엔 실패한 체크를 발견하는 즉시
  담당자를 붙였는데, 체크는 동시에 돌고 길이가 제각각이라(실측: `check`
  1분 22초 / `smoke` 2분 14초 / `review` 4분 19초) 짧은 게 먼저 깨지면 **나머지가 도는
  중에 메일이 갔다.** 열어봐도 체크가 아직 돌고 있어 뭘 판단하라는 건지 알 수 없고, 뒤늦게
  다른 체크가 더 깨져도 SHA 마커 때문에 두 번째 알림이 안 나가 실패 목록이 반쪽이 됐다.
  #55가 초록 쪽에서 고친 병("머지 버튼이 아직 비활성인 시점에 알림 도착")과 같은 것이고
  빨간 쪽만 남아 있었다. 이제 결론이 안 난 체크가 하나라도 있으면 판정을 미루고, `notify-ready`
  폴백이 워크플로우가 끝날 때마다 다시 본다 — **그래서 `notify-handle`을 쓰는 호출부는
  `workflow_run` 트리거를 반드시 달아야 한다**(아래 표 참고). 안 달면 리뷰 job의 90초
  창 안에 안 끝난 PR은 아무도 다시 판정하지 않는다. 영구 Pending 체크 하나가 알림을
  영원히 묻지 않도록, 30분(`STALE_PENDING_MIN`)을 넘겨 멈춘 체크는 기다리지 않는다.
...
- **Dependabot PR도 시크릿을 못 받는다** — fork PR과 같은 이유이고, 결과는 훨씬 나쁘다.
  ...
  2026-09-13 한 소비 저장소에서 실제로 그랬다(Dependabot PR 4건이 동시에 막혀 `agent-queued`
  ...
  **예외는 `claude-review.yml`의 `notify-ready` 하나다.** 그 job은 `workflow_run` 트리거라
  ...
  초록이든 **아무도 안 불린다**(2026-09-15 한 소비 저장소의 PR 셋에서 실측). 리뷰와 자동 수정은
...
> 없는 단일 브랜치 구조라 소비 저장소의 `claude.yml`을 그대로 옮겨 붙일 수도 없다(체크아웃
> 기준이 다르다). 다음에 이 질문이 다시 나오면: 필요해지면 `/install-github-app superokok/gh-workflows`를
> 실행하면 되고, 그전까지는 위 이유로 보류한다.

> **`enable-auto-merge`는 일부러 넣지 않았다.** 이 저장소는 **자기 자신이 게이트**라
> 거의 모든 변경이 위험 경로다 — 워크플로우는 물론이고 `scripts/skeleton/`도 잘못되면
> **새 프로젝트에만 조용히** 배달된다(2026-09-16 실측: 호출부가 `base-branch`를 빠뜨려
> 통합 브랜치가 `develop`이 아닌 저장소에서 게이트가 전부 skipped로 죽었다. actionlint도
> AI 리뷰도 못 잡았고, 실제로 붙여 보고서야 드러났다). 게다가 소비 저장소가 전부 `@main`을
> 핀하므로 **머지 즉시 전 프로젝트에 반영**된다.
>
> auto-merge를 켜도 자동으로 나갈 수 있는 건 문서 PR 정도인데, 실제 분포가 그렇지 않다
> (2026-09-16 PR 9건 중 문서만 바꾼 것 0건). 그래서 **머지는 사람이 한다.**
>
> 작성자 쪽 직렬화(앞 PR이 머지될 때까지 다음 작업을 못 시작)는 auto-merge가 아니라
> **스택 PR**로 푼다 — 다음 작업의 base를 앞 PR의 브랜치로 잡는다(정본 지침
> `work-loop.md`). 사람이 머지하는 횟수는 그대로지만, 모아서 순서대로 누르면 된다.
>
> 이 판단을 바꿀 조건은 하나다: **위험한 부류를 기계가 잡게 되는 것.**
> `scripts/check-skeleton.sh`가 그 첫 걸음이다(호출부가 재사용 워크플로우의 요구 입력을
> 실제로 넘기는지 검사). 그런 단언이 쌓이면 "스켈레톤만 건드린 PR"부터 자동 머지로 넘길 수 있다.
...
## 버전 · 릴리스 — 소비 저장소는 `@v1`을 핀한다

**머지와 배포를 분리한다.**

```
main 머지     → 아무 프로젝트도 바뀌지 않는다   (그래서 자동 머지한다)
v1 태그 이동  → 그때 전 프로젝트에 반영된다     (이것이 릴리스다)
```

`actions/checkout@v4`처럼 **움직이는 메이저 태그**다. 깨는 변경은 `v2`를 새로 만들고 소비
저장소를 하나씩 옮긴다 — 한 번에 전부 깨지지 않는 게 요점이다.

**태그는 `release` 라벨이 붙은 PR이 머지되면 자동으로 움직인다**(`self-release.yml`).
수동 dispatch로 두지 않은 이유: 잊으면 고친 게 프로젝트에 영영 안 가는데 **그 상태가
조용하다.** 라벨은 PR을 열 때 이미 "지금 나가야 하나"를 판단하는 자리라 따로 기억할 게 없다.
급하지 않은 변경은 라벨 없이 쌓아 두고 나중에 한 번에 내보낸다.

릴리스마다 `v1.<날짜>.<sha>` 고정 태그도 같이 남긴다 — 움직이는 태그만 있으면 "지난주
상태로"가 불가능하다. 되돌리기는 `git tag -f v1 <이전 sha>` 후 그 태그를 강제로 push면 된다.

**왜 `develop`을 두지 않나**: `develop`은 "배포라는 사건" 앞에 변경을 모아두는 버퍼다.
ref로 소비되는 저장소는 배포가 따로 없다 — 소비자가 핀을 옮기는 순간이 곧 릴리스다. 그러니
버퍼는 브랜치가 아니라 **핀(태그)**이어야 하고, `develop`을 두면 같은 일을 하는 버퍼가 둘이
되면서 릴리스 PR만 는다.
