...
- **재사용 워크플로우에서 `uses: ./...`(로컬 상대 경로 composite action)은 쓸 수 없다.**
  그 경로는 **호출부 저장소**를 가리키므로 다른 프로젝트에서 부르면 깨진다. 대신
  `uses: superokok/gh-workflows/.github/actions/<name>@main`처럼 **이 저장소를 명시한
  절대 경로**로 부르면 된다 — 원격 참조라 호출부의 체크아웃과 무관하다(`label-with-retry`,
  `notify-assignee-if-clean`이 그렇게 공유된다). 그래도 지나치게 짧거나(코드보다 `with:`가
  더 긴) 체크아웃 순서에 얽힌 로직은 그냥 인라인으로 둔다(`skip-paths-regex` 판정이 그렇다).
